...
public final class PolyMessagingClient: @unchecked Sendable {
    ...
    init(config: Configuration) {
        ...
        // PolyAI-internal envs (.dev, .staging) override bundle ID with the
        // env's known infra host. Customer-facing envs keep bundle ID as the
        // X-Host so customer-registered hosts route correctly.
        let resolvedHostId: String
        if let configured = config.hostIdentifier {
            resolvedHostId = configured
        } else if urls.preferDefaultHost {
            resolvedHostId = urls.defaultHostIdentifier
        } else {
