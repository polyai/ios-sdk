...
open class DevSettings: ObservableObject {
    ...
    /// Optional `X-Host` override. When nil, the host is derived from the
    /// selected environment (e.g. `.dev` → `jupiter-api.dev.polyai.app`), or
    /// inherited from the config passed to `initialize(_:)`.
    ...
    open func environmentDisplayName() -> String {
        ...
        case .staging: return "messaging.staging.poly.ai"
        case .dev: return "messaging.dev.poly.ai"
        case .cluster:
            let name = clusterName.trimmingCharacters(in: .whitespaces)
