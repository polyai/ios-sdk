...
actor ConnectionService {
    ...
    /// Force a synthetic 1006 close so the reconnect ladder engages
    /// immediately. Use when an out-of-band signal (OS-level offline
    /// detection, stale-socket suspicion after a long background) tells us
    /// the socket is effectively dead but the transport hasn't noticed yet.
    ///
    /// Unlike `disconnect()`, this preserves `shouldReconnect = true` so the
    /// existing reconnect ladder runs. The 1006 close flows through normal
    /// `handleClose` routing → `scheduleReconnect`, identical to a real
    /// WS keep-alive timeout — the consumer-facing status transitions are
    /// the same, just delivered seconds earlier.
    ...
    private func handleClose(_ event: ConnectionCloseEvent) {
        connectionStartedAt = nil
        // closeEvents fires for ALL closes — consumers may log or inspect.
        // .closed status, by contrast, is only emitted for terminal closes
        // (code 1000). Transient closes that trigger scheduleReconnect skip
        // .closed so consumers see open → reconnecting directly, matching
        // the Ably/Twilio pattern of clean state transitions for network
        // changes.
        closeEvents.emit(event)

        // 4003 = device-level network unavailability (DNS / offline / host
        // unreachable). The session_id is fine; we just can't reach the
        // server. Treat as transient, do NOT route to invalid-session refetch
        // — that would consume the invalid-session budget and corrupt
        // SessionState while the device is offline, breaking auto-recovery
        ...
        switch event.code {
        case 1000:
            // Terminal — server closed cleanly. Surface as .closed.
            statusChanges.emit(.closed(event))
            shouldReconnect = false
            logger.info("Normal close — no reconnect", metadata: nil)

        case 4000:
            // Replaced by a new connect() — the new attempt already emitted
            // .connecting. Skipping .closed avoids a stale flash.
        ...
        case 4001:
            logger.info("Server rejected session (4001)", metadata: nil)
            routeToInvalidSession()
