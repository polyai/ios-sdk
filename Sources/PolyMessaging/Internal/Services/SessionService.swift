...
actor SessionService {
    ...
    // debounced refetch completes. Debounce window +
    ...
    /// resumed from persisted storage). Tracks whether a session was created or
    /// drives `isResume` plumbing so we don't re-request `RequestPolyAgentJoin`
    /// on a session the server already considers "in flight".
    private(set) var wasSessionCreatedThisLaunch: Bool = false
    ...
    func createSession() async throws {
        ...
        do {
            ...
            wasSessionCreatedThisLaunch = true
    ...
    func resume() async throws {
        if let stored = store.load() {
            ...
            if withinTimeout,
               ...
               JWTValidator.isStructurallyValid(token) {
                ...
                wasSessionCreatedThisLaunch = false
        ...
        // createSession() sets wasSessionCreatedThisLaunch = true.
