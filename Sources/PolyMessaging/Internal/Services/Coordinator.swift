...
actor Coordinator {
    ...
    func start() async throws {
        ...
        // already has joined. Resets state for the joined session.
        let isResume = !(await sessionService.wasSessionCreatedThisLaunch)
    ...
    /// `sendTyping()` calls. Uses a 3-second throttle and adds the
