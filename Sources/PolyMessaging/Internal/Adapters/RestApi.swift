...
actor RestApi: RestApiPort {
    ...
    func obtainAccessToken() async throws -> AccessTokenResponse {
        // Clear cached credentials before re-issuing so a failed
        // in-flight token can't leak into a fresh fetch.
        accessToken = nil
    ...
    func createSession(context: SessionContext) async throws -> SessionCreated {
        // Reset the cached access token so a stale handle from a previous
        // session doesn't leak into the new creation. Restored on success.
