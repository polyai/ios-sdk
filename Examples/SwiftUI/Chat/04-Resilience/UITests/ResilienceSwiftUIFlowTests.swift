...
/// README), driven against the live backend.
///
/// 04-Resilience's headline features — the offline banner, loading skeleton,
/// and terminal error screen — all require network manipulation that XCUITest
/// cannot drive (airplane mode / killing the socket from a test process).
/// Those paths are covered deterministically in ChatSessionTests. Here we
/// assert the one thing that is observable end-to-end: the resilient chat still
/// connects and round-trips a message against the live agent.
final class ResilienceSwiftUIFlowTests: XCTestCase {
    ...
    /// 04-Resilience auto-connects (no connect screen) — wait for the composer.
