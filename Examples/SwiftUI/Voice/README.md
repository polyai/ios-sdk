...
A one-screen tap-to-call demo on **`PolyVoice`** — the smallest thing that makes a WebRTC voice call:
`PolyVoice.call(...)`, observe `call.states`, start / mute / end.

## Run it

1. Open `VoiceSwiftUI.xcodeproj` and set your team under **Signing & Capabilities**.
2. In `ContentView.swift`, fill in your connector from **Agent Studio › Connector Settings** —
...
- `for await state in call.states` — `.connecting → .connected → .ended / .failed`
